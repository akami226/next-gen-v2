...
import { X, Copy, Check, Car, Palette, Disc3, PanelTop, AudioLines } from 'lucide-react';
...
const SOCIALS = [
  ...
  },
  {
    name: 'Facebook',
    color: '#1877F2',
    getUrl: (_text: string) => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`,
    icon: () => (
...
export default function ShareModal({ isOpen, onClose, buildConfig }: ShareModalProps) {
  const [copied, setCopied] = useState(false);

  const buildText = `Check out my NextGen build:\n${buildConfig.car}\nWrap: ${buildConfig.wrap}\nWheels: ${buildConfig.wheels}\nTint: ${buildConfig.tint}\nExhaust: ${buildConfig.exhaust}\n\n${window.location.href}`;
  ...
  const handleShare = useCallback((getUrl: (text: string) => string) => {
    window.open(getUrl(buildText), '_blank', 'noopener,noreferrer,width=600,height=500');
  }, [buildText]);
          ...
          <div className="relative flex items-start justify-center min-h-full py-8 px-4">
            ...
            >
              ...
              <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 mb-5">
                <p className="text-[11px] font-semibold text-white/50 uppercase tracking-wider mb-3">Build Summary</p>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                {SOCIALS.map((social) => (
                  <button
                    ...
                    className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.06] text-white/70 hover:border-white/[0.12] hover:bg-white/[0.06] transition-all"
                  >
              ...
              <button
                ...
                className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl border text-sm font-semibold transition-all duration-200 ${
                  copied
                    ...
                    : 'bg-white/[0.06] border-white/[0.08] text-white/70 hover:bg-white/[0.1]'
                }`}
              >
